// The real name of the language is oOo CODE
function _brainfuck(program,input){
    var stack=[];
    var matches={};
    var ip=0;
    var tape=[];
...
function ooocode(program,input){
    var bf='',clean='';
    ...
    for(let i=0;i*3<clean.length;i++){
        var binary=0;
        for(let j=0;j<3;j++){
            binary=binary*2+('qwertyuiopasdfghjklzxcvbnm'.includes(clean[i*3+j])?0:1);
        }
        bf+='><[]-+.,'[binary];
    }
    return _brainfuck(bf,input);
}